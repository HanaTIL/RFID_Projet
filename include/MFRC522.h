...
typedef enum PICC_Command{
	...
	typedef enum MFRC522_statusCodes{
		...
		MFRC522_STATUS_AUTHEN ,
		MFRC522_STATUS_NOT_AUTHEN

	}MFRC522_statusCodes_t;
...
void MFRC522_WriteRegister_Multi(PCD_Register_t reg, byte count, byte *value);
...
MFRC522_statusCodes_t PCD_CalculateCRC(byte *data, byte length,byte *result);	///< In: Pointer to the data to transfer to the FIFO for CRC calculation.\
